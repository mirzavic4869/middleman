      ...
      {datas.length !== 0 ? (
        loading ? (
          ...
          <div className="grid grid-cols-1 m-3 gap-2 md:grid-cols-2 lg:grid-cols-3">
            {datas.map((data) => (
              <OutBound key={data.product_id} id={data.product_id} name={data.product_name} unit={data.unit} qty={data.qty} fnDeleteData={deleteData} loading={loading} amount={data.amount} handleQty={(type) => handleQty(data, type)} />
            ))}
          </div>
        )
      ) : (
        <div className="flex  justify-center items-center text-lg md:text-3xl font-Roboto font-bold text-slate-700/20">Add your outbound</div>
      )}
